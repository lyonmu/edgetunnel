...
export async function generateMixedSubscription(context: RequestContext): Promise<string> {
  ...
  const addresses = await getOptimizedAddresses(context, config);

  for (const addr of addresses) {
    const node = generateNodeLink(addr, config, context);
    if (node) {
      nodes.push(node);
    }
  }
  ...
  if (!context.ua.includes('mozilla') || url.searchParams.has('b64') || url.searchParams.has('base64')) {
...
async function getOptimizedAddresses(
  ...
  return generateRandomIPs(request);
}
...
function generateRandomIPs(request: Request): string[] {
...
function generateNodeLink(
  ...
  const match = address.match(/^(\[[\da-fA-F:]+\]|[\d.]+|[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)*)(?::(\d+))?(?:#(.+))?$/);
