...
import { httpConnect } from '../../../src/networking/proxy-connectors';
...
describe('HTTP CONNECT connector integration', () => {
  it('accepts arbitrarily fragmented headers and preserves tunneled response bytes', async () => {
    let request = '';
  ...
  it('rejects proxy errors without echoing credentials', async () => {
    const fixture = await startTcpServer(async (socket) => {
      socket.once('data', () => {
